				...
				if ((in_menu($id) || (sizeof($post->ancestors) > 0)) && !(is_search())) {
					...
					if ( is_page() ) {
						...
						if ( sizeof($post->ancestors) == 0 && $children ) {
							...
							echo '<li class="current_page_item top_level_page"><a href="/' . get_post($ptg)->post_name . '">' . get_post($ptg)->post_title . '</a></li>';
						} elseif ( sizeof($post->ancestors) > 0 ) {
							// Sub-page
							echo '<li class="top_level_page"><a href="/' . get_post($ptg)->post_name . '">' . get_post($ptg)->post_title . '</a></li>';
						}
